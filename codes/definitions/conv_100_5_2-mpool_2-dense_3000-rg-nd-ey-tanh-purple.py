...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  4 22:10:52 2018
...
def get_cost_updates(network, input_var, output, learning_rate, **kwargs):
    ...
    mini = T.min(flat_output, axis = 1).reshape((batch_size, 1)).repeat(3072, axis = 1)
    maxi = T.max(flat_output, axis = 1).reshape((batch_size, 1)).repeat(3072, axis = 1)
    flat_output = (flat_output - mini) / (maxi - mini)
