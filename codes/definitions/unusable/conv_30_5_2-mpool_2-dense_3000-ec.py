...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  4 22:32:15 2018
...
def define_model(input_var, **kwargs):
    ...
    deconv1_flatten = layers.ReshapeLayer(
        incoming = deconv1,
        shape = (input_var.shape[0], 1, 1, 3 * image_size * image_size),
    )
    
    grey_with_deconv1_flatten = layers.ConcatLayer(
        incomings = [deconv1_flatten, greyscale_input],
        axis = 1,
    )

    output = layers.DenseLayer(
        incoming = grey_with_deconv1_flatten,
        num_units = 3 * image_size * image_size,
        nonlinearity = lasagne.nonlinearities.linear,
    )
