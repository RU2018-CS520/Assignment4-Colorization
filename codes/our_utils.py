...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 00:22:20 2018
...
# package for python to manipulate images
...
def save_image_from_array(arr, file_name = 'image.png', directory = 'plots'):
    '''
    arr must be numpu.ndarray: (channel, height, width) or (1, channel, height, width)
    there can be 3 channels or 1 channel
    '''
    if (arr.dtype != 'uint8'):
        arr = np.array(np.floor(arr * 256), dtype = 'unit8')
...
def get_greyscale(images, random_greyscale = False, rng = None, rgb = False):
    '''
    images are 4D array of images (4D tensor or np.ndarray): (index, channel, height, width)
    if use_random_params is True, must provide rng
    
    if rgb = True, the output will have 3 channels, otherwise 1 channel
    
    note: when images is T.tensor4 rng must be theano.tensor.RandomStreams, np.array rng must be numpy.random
    '''
    cr = 0.29
    cg = 0.6
    cb = 0.11
    ...
    res = cr * images[0:cnt, 0:1, 0:h, 0:w] + cg * images[0:cnt, 1:2, 0:h, 0:w] + cb * images[0:cnt, 2:3, 0:h, 0:w]
    if rgb:
...
def print_samples(images, forward, model_name, epoch, suffix = '', columns = 1, directory = 'plots'):
    '''
    images are 4D array of images: (index, channel, height, width)
    forward is a theano.function which maps images to outputs
    note: images can be tensor4 for np.ndarray
    '''
...
def plot_filters(filters, model_name, epoch, suffix = '', max_num_filters = 100, columns = 1, repeat = 5, directory = 'plots'):
    ...
    one filters size = repeat * kernel1 size
    '''
...
def save_model(network, epoch, model_name, learning_rate = 0.0, directory = 'models'):
    '''
    save networks parameters, epoch, learning_rate
    '''
...
def load_model(network, file_name, directory = 'models'):
    '''
    loads saved network and returns dictionary with keys epoch, learning_rate
    
    loads from root_dir/models/file_name
    '''
